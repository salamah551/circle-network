...
/**
 * Plain-text Email #0: Founder Intro (Optional First Touch)
 * Toggleable via CAMPAIGN_ENABLE_INTRO_PLAINTEXT env var
 * - Reply-friendly plain text format
 * - No UTM parameters or click tracking
 * - Single CTA link only
 * - List-Unsubscribe headers included
 */
export const sendPlainTextIntroEmail = async ({ to, name, inviteCode }) => {
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://thecirclenetwork.org';
  const inviteLink = `${appUrl}/?code=${inviteCode}`;
  const unsubscribeUrl = `${appUrl}/unsubscribe?email=${encodeURIComponent(to)}&token=${inviteCode}`;
  
  // Use INVITES_FROM_EMAIL if provided, otherwise fallback to SENDGRID_FROM_EMAIL
...
`,
    ...
    // Disable click tracking for Email #0
    trackingSettings: {
  ...
  try {
    ...
    console.error('Error sending plain-text intro email:', error);
    return { success: false, error };
  }
};

export default { sendInvitationEmail, sendPlainTextIntroEmail };
      ...
      <body>
        ...
        <div class="content">
          ...
          ${isFoundingMember ? `
            ...
            <p>You're now part of an exclusive group that will shape the future of this community. Your lifetime price of <strong>$2,497/year</strong> is locked in forever—that's a permanent <strong>50% discount</strong> off standard pricing.</p>
            
            <div class="feature-box">
              <h3 style="margin-top: 0; color: #D4AF37;">Your Founding Member Benefits:</h3>
              <ul style="margin: 10px 0; padding-left: 20px;">
                <li><strong>Lifetime Price Lock:</strong> $2,497/year forever (50% off)</li>
