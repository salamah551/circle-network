...
//   SUPABASE_SERVICE_ROLE_KEY — Service role key for admin DB writes (server-only)
//
 ...
 */
export async function processArcRequest({ requestId, prompt, type, attachmentTexts }) {
  ...
  if (!apiKey) {
    await supabase.from('arc_requests').update({
      status: 'failed',
      result_md: '**Error:** AI processing is not configured. Please contact support.',
