...
import { 
  ArrowLeft, Search, Download, UserCheck, UserX, 
  Mail, Linkedin, Crown, Loader2, 
  MoreVertical, Check, X, Trash2, CheckSquare, Square
} from 'lucide-react';
...
export default function AdminMembersPage() {
  ...
  const exportSelectedMembers = async () => {
    try {
      ...
      a.href = url;
      a.download = `circle-members-${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
  ...
  const bulkUpdateStatus = async (newStatus) => {
    ...
    if (!confirm(`Update ${selectedMembers.length} members to ${newStatus}?`)) return;
  ...
  const bulkDelete = async () => {
    ...
    if (!confirm(`Delete ${selectedMembers.length} members permanently?`)) return;
          ...
          {selectedMembers.length > 0 && (
            <div className="flex items-center gap-2 p-4 bg-zinc-900 rounded-lg border border-zinc-800">
              ...
              <button
                ...
                className="px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 rounded-lg hover:bg-emerald-500/20 transition-colors text-sm disabled:opacity-50"
              >
                Activate
              </button>
              <button
                ...
                disabled={bulkActionLoading}
                className="px-3 py-1.5 bg-amber-500/10 border border-amber-500/30 text-amber-400 rounded-lg hover:bg-amber-500/20 transition-colors text-sm disabled:opacity-50"
              >
                Deactivate
              </button>
              <button
                onClick={bulkDelete}
                disabled={bulkActionLoading}
                className="px-3 py-1.5 bg-red-500/10 border border-red-500/30 text-red-400 rounded-lg hover:bg-red-500/20 transition-colors text-sm disabled:opacity-50"
              >
                Delete
              </button>
              {bulkActionLoading && (
