...
if (!adminEmails.includes(profile?.email)) {
  ...
  return (
    <div className="min-h-screen bg-black text-white">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl max-w-md w-full p-8">
            ...
            <div className="space-y-4 mb-6">
              ...
              <div>
                ...
                />
                <p className="text-xs text-zinc-500 mt-1">Max 100 emails/day (SendGrid free tier)</p>
              </div>
            </div>
      ...
      {/* Upload Recipients Modal */}
      {showUploadModal && selectedCampaign && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl max-w-md w-full p-8">
            <h3 className="text-2xl font-bold text-white mb-6">Upload Recipients</h3>
            
            <div className="mb-6">
              <label className="block text-sm font-medium text-zinc-300 mb-2">
                CSV File (First Name, Last Name, Email)
              </label>
              <input
                type="file"
                accept=".csv"
                onChange={handleFileUpload}
                className="w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-lg text-white file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:bg-amber-500 file:text-black file:font-medium hover:file:bg-amber-600"
              />
              <button
                onClick={downloadCSVTemplate}
            ...
            {uploadProgress > 0 && (
              <div className="mb-6">
                ...
                <p className="text-sm text-zinc-400 mt-2 text-center">{uploadProgress}%</p>
              </div>
            )}
